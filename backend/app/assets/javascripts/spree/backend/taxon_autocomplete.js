...
$.fn.breadcrumbSourceAutocomplete = function () {
  'use strict';
  ...
  this.select2({
      ...
      initSelection: function (element, callback) {
        ...
        Spree.ajax({
          ...
          data: {
            ids: ids,
            per_page: 1, // Fetch only one
            without_children: true
          },
          success: function (data) {
            callback(data['taxons'][0]); // Pass a single taxon
          }
        });
      },
      ajax: {
...
Spree.ready(function () {
  ...
  $('#product_breadcrumb_source_id, .taxon_picker').breadcrumbSourceAutocomplete();
});
